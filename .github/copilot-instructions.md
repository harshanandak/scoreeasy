...
3. **Writing TypeScript into `src/`.** `src/` is 173 JS/JSX files and zero `.ts`; `tsconfig.json` runs with `strict: false` and `checkJs: false`. Adding a `.tsx` file gets no type checking and breaks the convention.
