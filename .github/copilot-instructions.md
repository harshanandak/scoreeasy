...
> Sources: AGENTS.md, CODING_STANDARDS.md, docs/agents/forge-workflow.md
...
**Review feedback is advisory** except four classes that must be fixed in the active PR: secrets, injection, a broken build, and **documentation that asserts something the code does not do**. Anything else worth doing becomes a follow-up issue, not more commits on this branch. (Pre-user shipping regime, decided 2026-08-16. The reviewer-bot policy is versioned in `.coderabbit.yaml`; its request-changes workflow is disabled so this advisory rule remains authoritative, and this paragraph must stay identical to the "Shipping regime" bullet in `AGENTS.md`.)

Sonar and other scanners are graded on the same four classes, not on their own severity labels: a Blocker that is a real build break or injection gets fixed here, a Blocker that is an advisory code-quality finding gets filed. Severity says how much it matters, not who fixes it when.
...
- `AGENTS.md` at the repo root is the single source of truth. **This file is not it** —
  `docs/agents/forge-workflow.md` is one of the sources `bun run sync:agents` composes
  into the harness mirrors.
- Tool setup and enforcement details live in `AGENTS.md` and `CODING_STANDARDS.md`.
  There are no `docs/TOOLCHAIN.md` or `docs/VALIDATION.md` files in this repo — if you
  were sent here looking for them, they never existed.
